...
def test_ttl_to_nx():
    ...
    # Check graphs created with different colour scale are the same if not comparing attributes
    nx_graph_digmap = ttl_to_nx(triples=triples)
    assert nx.is_isomorphic(nx_graph_digmap,nx_graph_age)
    
    # Check graphs created with different colour scale are are different when also comparing attributes
